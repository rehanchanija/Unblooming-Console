...
export default function ProductPage() {
  ...
  return (
    <main className="bg-bg text-gray-900">
      <section className="section-pad mx-auto max-w-7xl px-6">
        <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-gray-900/70">
              {product.description}
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            <Link href="/" className="rounded-full bg-white/10 px-5 py-3 text-sm text-gray-900 transition hover:bg-white/20">
              Back to Home
            </Link>
          </div>
        </div>
          ...
          <div className="space-y-8">
            <Reveal>
              <div className="grid gap-5 rounded-3xl border border-borderc bg-black/60 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.45)]">
                <div className="relative overflow-hidden rounded-[2rem] border border-gray-200 bg-[#090909] p-6">
                  <Image
            ...
            <Reveal direction="up">
              <div className="rounded-3xl border border-borderc bg-black/60 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
                ...
                <ul className="mt-6 space-y-3 text-sm text-gray-900/70">
            ...
            <Reveal>
              <div className="rounded-[2rem] border border-borderc bg-black/70 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.45)]">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm uppercase tracking-[0.35em] text-gray-900/50">Price</p>
                    <p className="mt-2 text-4xl font-bold text-gray-900">${product.price}</p>
                  </div>
                ...
                <div className="mt-6 space-y-4 rounded-3xl bg-white/5 p-5">
                  <div className="flex items-center justify-between gap-3 rounded-3xl bg-white/5 p-4">
                    <div>
                      <p className="text-sm text-gray-900/70">Quantity</p>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="rounded-full bg-white/10 px-3 py-2 text-xl text-gray-900 transition hover:bg-white/15"
                  ...
                  <button
                    ...
                    className="w-full rounded-full bg-gradient-to-r from-primary to-secondary px-6 py-3 text-sm font-semibold text-gray-900 transition hover:brightness-110"
                  >
                    Add to cart
                  </button>

                  <div className="rounded-3xl bg-white/5 p-4 text-sm text-gray-900/70">
                    <p>Cart items: <span className="font-semibold text-gray-900">{cartCount}</span></p>
                    <p className="mt-2">Order total: <span className="font-semibold text-gray-900">${subtotal}</span></p>
            ...
            <Reveal direction="up">
              ...
              >
                ...
                <div className="grid gap-4">
                  <label className="space-y-2 text-sm text-gray-900/70">
                    Full name
                    <input
                      value={delivery.fullName}
                      onChange={(event) => handleChange("fullName", event.target.value)}
                      className="w-full rounded-3xl border border-gray-200 bg-white/5 px-4 py-3 text-gray-900 outline-none transition focus:border-accent"
                ...
                <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-900/70">Payment method</p>
                ...
                {status && (
                  <p className={`mt-5 rounded-3xl border border-gray-200 bg-white/5 px-4 py-3 text-sm ${
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              {product.features.map((feature) => (
                <div key={feature.label} className="rounded-3xl border border-borderc bg-black/60 p-6">
                  <p className="text-sm uppercase tracking-[0.35em] text-gray-900/50">{feature.label}</p>
                  <p className="mt-4 text-lg font-semibold text-gray-900">{feature.value}</p>
                </div>
              ))}
            </div>
          </Reveal>
